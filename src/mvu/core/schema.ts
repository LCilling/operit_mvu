 ...
 * core/schema.ts
 *
 * 从 MagVarUpdate 上游 `src/function/schema.ts` 移植（commit 0a730cd…b977c24c）。
 * 保留 EXTENSIBLE_MARKER、generateSchema、getSchemaForPath、reconcileAndApplySchema、
 * cleanUpMetadata 的行为与命名；仅替换 lodash（_）/klona 引用为 port/util 等价。
 */
...
import { klona, has, isDate, isObject } from '../port/util';
...
/**
 * 递归地为数据对象生成一个模式。
 * @param data - 要为之生成模式的数据对象 (stat_data)。
...
export function getSchemaForPath(
  ...
  const pathSegments = toPathLocal(path);
...
// 局部引入 toPath，避免循环依赖
import { toPath as toPathLocal } from '../port/util';
