...
当前公开版本没有 v1 迁移和双格式读取。新数据集不包含假角色、假聊天、假消息或假变化记录，也没有恢复演示数据的运行时命令。

当前交付物是单一外部 `operit_mvu-2.0.1.toolpkg`。静态 WebView 页面、编译后的 main runtime、v2 引擎、文档和上游许可都在同一包中；MVU 不打入 APK，也没有子包、辅助 APK、第二个接口包或独立前端目录，页面不提供插件专用退出按钮。
