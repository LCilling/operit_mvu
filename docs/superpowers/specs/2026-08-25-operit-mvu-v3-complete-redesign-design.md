...
“高级”中的完整数据备份继续负责整库迁移和灾难恢复；“配置 / 字段”中的字段模板导入导出只负责便携地复用一个或多个字段，两者使用不同文件类型、提示和 IPC，不能互相覆盖。

字段模板采用带版本和校验摘要的 `operit-mvu-field-template` 格式。默认包含字段定义、上下限、阶段、外观和字段专属的自然变化/每轮变化/AI 更新配置，不携带当前数值、变化记录、规则、条件或效果组。用户可在导出抽屉中显式勾选“携带选中角色的当前值”；任何被引用但未包含的规则、条件或效果只写入依赖清单，不静默创建或改写本地对象。

角色独立字段的导出抽屉按字段分组显示角色启用矩阵：每个角色具有“包含配置”和“携带当前值”两个明确开关，角色较多时复用搜索选择框。群组共享字段使用同样的群组矩阵；全局字段不显示无意义的角色开关；会话字段默认映射当前会话，高级映射默认收起。导出成功后由宿主保存文件，界面显示文件名、保存位置和实际包含的字段/角色数量。

导入先解析并校验文件，再在同一抽屉内显示三步预览，不创建独立一级页面：

1. **内容预览**：字段、配置组成、来源版本、依赖和是否携带数值。
2. **冲突处理**：默认“创建副本”；也可选择“仅更新配置并保留本地数值/绑定”或“替换所选内容”。不得按名称自动覆盖。
3. **角色与群组映射**：先按稳定外部标识匹配，再按唯一名称提出建议；重名或缺失对象必须由用户搜索选择。每个目标角色都有“启用此字段”开关，默认启用所有已明确映射的角色，未启用角色不创建绑定。携带数值时逐角色选择“使用导入值 / 保留本地值 / 使用字段默认值”，导入值超出新范围时在提交前显示换算或截断预览。

导入提交使用客户端 `expectedRevision` 和单次原子事务：预览与当前修订不一致时拒绝保存并要求重新载入；任一字段、依赖、角色映射或数值校验失败时全部回滚。结果页明确列出创建、更新、跳过和待修复项目，并提供进入字段列表筛选结果的入口。

建议 IPC 契约：

```ts
previewFieldTemplateImport({ json }): FieldTemplatePreview
importFieldTemplate({ json, expectedRevision, decisions }): FieldTemplateImportResult
exportFieldTemplate({ fieldIds, options, actorSelections }): { fileName, savedPath, summary }
```

解析器限制文件字节数、字段数、角色映射数、文本长度和未知键；模板不执行脚本，不信任来源 ID，不允许覆盖完整数据备份。字段和角色较多时，选择与映射全部使用有界查询及可搜索选择框。
...
4. **高级**：完整数据备份导入导出、外观、v2/v3 迁移、默认条件恢复和诊断。便携字段模板导入导出仍归属“配置 / 字段”。
