...
MVU 通常以最多 32 行为一段读取记录。发现该标记时，必须丢弃不完整结果，并对当前行区间做有界二分后继续调用 `Tools.Files.readPart`；32 行初始区间最多拆分 5 层，不得解析部分 JSON，也不得回退到会一次性分配整个记录文件的 `Tools.Files.read`。单条持久化记录最多允许 32,000 个 UTF-16 代码单元：写入前必须校验，既有超长单行必须明确拒绝。该策略不引入新的宿主接口。若未来返回结构增加 `truncated: boolean`，固定标记仍应保留至少一个兼容周期。
