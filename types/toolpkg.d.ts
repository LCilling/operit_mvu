...
export namespace ToolPkg {
    ...
    export interface ChatMessageEventPayload extends JsonObject {
        chatId: string;
        /** Decimal Room primary key, encoded as a string to preserve 64-bit precision. */
        messageId: string;
        orderIndex: number;
        /** Null when the selected answer is the original variant at index 0. */
        variantId: string | null;
        /** Authoritative selected variant index read from Room. */
        variantIndex: number;
        /** Stable character-card identity resolved without display-name guessing. */
        actorCharacterCardId: string | null;
        /** Stable character-group identity for group-bound chats. */
        ...
        /** True when the persisted user message or completed AI answer is ready for state settlement. */
        isComplete: boolean;
        timestamp: number;
        sender: string;
        roleName: string;
    ...
    export type UiRouteTopBar = "host" | "hidden";
    ...
    export type ChatContextPromptType = "character_card" | "character_group";

    export interface ChatContextCharacterSnapshot extends JsonObject {
    ...
    export interface ChatContextMemberSnapshot extends JsonObject {
        characterCardId: string;
        name: string;
        orderIndex: number;
        /** Host avatar URI; a UI surface still needs permission for its URI scheme. */
    ...
    export interface ChatContextSnapshot extends JsonObject {
        chatId: string | null;
        /** Null for an explicitly requested chat whose persisted binding is absent or ambiguous. */
        activePrompt: ChatContextActivePromptSnapshot | null;
         ...
         * current host group manager does not expose a persistent current speaker.
         */
    ...
    /** Strict JSON Schema requested for an isolated system-model completion. */
    export interface SystemModelJsonSchema extends JsonObject {
        /** Schema identifier accepted by OpenAI-compatible structured-output APIs. */
        name: string;
        /** Root schema. It must describe a JSON object. */
        schema: JsonObject;
    }

    export interface SystemModelCompletionRequest extends JsonObject {
        systemPrompt: string;
        userPrompt: string;
        /**
         * When present, the host requests provider-native strict structured output.
         * Providers that cannot honor the schema fail the completion instead of returning
         * unconstrained text.
         */
        jsonSchema?: SystemModelJsonSchema;
    }

    export interface SystemModelCompletionResult extends JsonObject {
        text: string;
        providerModel: string;
    }
    ...
    export interface SystemModelApi {
        ...
        /** Isolated one-shot completion; the host cancels requests that exceed 150 seconds. */
        complete(request: SystemModelCompletionRequest): Promise<SystemModelCompletionResult>;
    }
